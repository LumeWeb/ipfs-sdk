...
import (
	...
	"go.lumeweb.com/ipfs-sdk/pkg/archive/internal"
)
...
func (r *ArchiveRegistry) DetectFormat(reader io.Reader) (Format, error) {
	...
	currentPos, err := internal.PrepareReaderPreservePos(seeker)
	if err != nil {
	...
	if err != nil && err != io.EOF && err != io.ErrUnexpectedEOF {
		...
		if restoreErr := internal.RestoreReaderPos(seeker, currentPos); restoreErr != nil {
	...
	if detectedFormat == FormatUnknown || detectedFormat == FormatFile {
		...
		if err != nil {
			// Restore original position on error
			if seekErr := internal.RestoreReaderPos(seeker, currentPos); seekErr != nil {
	...
	if err := internal.RestoreReaderPos(seeker, currentPos); err != nil {
