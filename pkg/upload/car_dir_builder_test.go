...
// TestNewCARBuilder_NilBlockstore tests NewCARBuilder with nil blockstore/dagService
func TestNewCARBuilder_NilBlockstore(t *testing.T) {
	// When blockstore and dagService are nil, NewCARBuilder creates defaults
	bs, dagService := NewDAGServiceWithMemoryLimit(10000)
	generator := NewUnixFSNodeGenerator(WithUnixFSNodeDAGService(dagService), WithUnixFSNodeBlockstore(bs))
	builder := NewCARBuilder(nil, nil, generator)
	
	if builder == nil {
		t.Fatal("NewCARBuilder with nil params returned nil")
	}

	// Should have created default blockstore and dagService
	if builder.bs == nil {
		t.Error("blockstore should be created from default")
	}

	if builder.dagService == nil {
		t.Error("dagService should be created from default")
	}

	// Verify it works with the default setup
	memFS := fstest.MapFS{
		"file.txt": {Data: []byte("test")},
	}

	summary, err := builder.BuildSummary(context.Background(), memFS, true)
	if err != nil {
		t.Fatalf("BuildSummary failed: %v", err)
	}
