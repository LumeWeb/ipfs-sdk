...
func TestLRUBlockstore_PutMany(t *testing.T) {
	...
	// Put multiple blocks at once
	blocks := []blocks.Block{blk1, blk2, blk3}
	...
	if bs.Len() != 3 {
		t.Errorf("Expected 3 blocks, got %d", bs.Len())
	}
	...
	// Check total size - note: size includes raw data (not CID bytes)
	expectedSize := uint64(len(data1) + len(data2) + len(data3))
	if bs.Size() != expectedSize {
		t.Errorf("Expected size %d, got %d", expectedSize, bs.Size())
	}
}

func TestLRUBlockstore_PutMany_Empty(t *testing.T) {
	bs := NewLRUBlockstore(100)
	ctx := context.Background()

	// Put empty slice of blocks
	err := bs.PutMany(ctx, []blocks.Block{})
	if err != nil {
		t.Fatalf("PutMany with empty blocks failed: %v", err)
	}

	if bs.Len() != 0 {
		t.Errorf("Expected 0 blocks, got %d", bs.Len())
	}
}
