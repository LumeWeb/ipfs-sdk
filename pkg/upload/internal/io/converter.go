...
// Otherwise, a wrapper is created that provides ByteReader functionality.
...
// ToByteReadSeeker converts an io.Reader to a ByteReadSeeker.
// ByteReadSeeker is an interface that combines io.ReadSeeker and io.ByteReader.
// If the reader already implements ByteReadSeeker, it is returned unchanged.
// If it implements io.ReadSeeker, a wrapper is created that adds ByteReader.
// Otherwise, a wrapper is created that provides both interfaces.
...
// ToReadSeeker converts an io.ReaderAt to an io.ReadSeeker.
// If the ReaderAt already implements io.ReadSeeker, it is returned unchanged.
// Otherwise, a wrapper is created that provides ReadSeeker functionality using ReaderAt.
...
// ToReaderAt converts an io.ReadSeeker to an io.ReaderAt.
// If the ReadSeeker already implements io.ReaderAt, it is returned unchanged.
// Otherwise, a wrapper is created that provides ReaderAt functionality using ReadSeeker.
