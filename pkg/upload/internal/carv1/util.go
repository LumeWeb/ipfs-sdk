...
// ReadNodeHeader reads the header of a node from a CARv1 stream.
// It returns the CID of the node and the length of the data to be read.
// The zeroLenAsEOF parameter controls whether a zero-length section is treated as EOF.
// The maxReadBytes parameter limits the maximum number of bytes to read.
...
func ReadNodeHeader(r io.Reader, zeroLenAsEOF bool, maxReadBytes uint64) (cid.Cid, uint64, error) {
	...
	br := internalio.ToByteReader(limitReader)
	// Cast to io.Reader since readerPlusByte implements both
	n, c, err := cid.CidFromReader(br.(io.Reader))
	if err != nil {
		return cid.Cid{}, 0, err
	}
